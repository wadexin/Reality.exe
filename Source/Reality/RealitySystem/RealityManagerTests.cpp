...
bool FRealityManagerSubsystemTest::RunTest(const FString& Parameters)
{
	...
	TestTrue(TEXT("Time has a configured rule"), Settings->FindBaseSuspicion(Tag(TEXT("Cheat.Time")), TimeValue));
	...
	TestEqual(TEXT("Time prototype tuning is 20"), TimeValue, 20.0f);
	...
	TestFalse(TEXT("A known but untuned semantic tag is rejected"), Manager->ProcessCheatEvent(Event(DelegateRecorder, TEXT("Context.Blackout"))));
